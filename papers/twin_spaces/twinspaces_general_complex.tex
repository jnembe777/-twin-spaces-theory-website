...
Every construction in this paper rests on a single idea: a symmetry does not merely \emph{preserve} an
operation, it \emph{transports} it. Conjugating a base operation $\alpha$ by a bijection $\phi$ produces a new
operation $\odot_\phi$ that is abstractly isomorphic to $\alpha$ yet generically distinct from it as a concrete
operation on the fixed set $E$. Letting $\phi$ range over a group $G$ spreads the single operation $\alpha$ into
an orbit of operations, and the geometry of that orbit---which $\phi$ fix $\alpha$, and which deform it---is
exactly the kernel--deformation dichotomy formalised in Section~\ref{sec:kernel}. This orbit is the right-coset
space $\Ker(\alpha)\backslash G$ that, in the companion foundational papers, carries the deformation cohomology
of the structure.
...
Take $E = \RR_{>0}$, $\alpha = +$ (addition), $\phi = \log$ (so that $\phi(xy) = \phi(x) + \phi(y)$):
\begin{center}\small
...
\begin{equation}
\Twin(\alpha) := \Ker(\alpha) \backslash G
\end{equation}
It parameterizes the \emph{physically distinct} twin operations: $g_1$ and $g_2$ produce the same operation iff $g_1 g_2^{-1} \in \Ker(\alpha)$, i.e.\ iff they lie in the same \emph{right} coset $\Ker(\alpha)\,g$. (The deformation space is thus the right-coset space $\Ker(\alpha)\backslash G$; it coincides with $G/\Ker(\alpha)$ when $\Ker(\alpha)\trianglelefteq G$.)
\end{definition}
...
\begin{proof}
$\mathfrak{ker}(\alpha) = \ker(\xi \mapsto \mathcal{L}_\xi\alpha)$ is the kernel of a linear map from $\mathfrak{g}$ to the space of bilinear maps on $E$. It is a Lie subalgebra: the Lie derivative is a Lie-algebra homomorphism,
$\mathcal L_{[\xi_1,\xi_2]}=[\mathcal L_{\xi_1},\mathcal L_{\xi_2}]$, so $\mathcal L_{\xi_1}\alpha=\mathcal L_{\xi_2}\alpha=0$
forces $\mathcal L_{[\xi_1,\xi_2]}\alpha=\mathcal L_{\xi_1}\mathcal L_{\xi_2}\alpha-\mathcal L_{\xi_2}\mathcal L_{\xi_1}\alpha=0$.
Any vector-space complement $\mathfrak{def}$ completes the splitting; note that $\mathfrak{def}$ is not canonical
and is generally \emph{not} a subalgebra, since the bracket of two deformations may regenerate a symmetry.
\end{proof}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item For a metric $g$ on a manifold: $\Ker(g) = \Isom(\mathcal{M}, g)$ (isometry group), and the kernel elements are exactly the Killing vector fields.
\end{enumerate}
\end{theorem}


% ============================================================================
\begin{example}[Deformations of a bilinear form are metrics]\label{ex:metrics}
Let $E=\RR^n$, let $\alpha=q$ be a fixed nondegenerate symmetric bilinear form, and let $G=\GL(n,\RR)$ act
linearly. An element $g$ lies in the kernel iff $q(gx,gy)=q(x,y)$ for all $x,y$, i.e.\ iff $g\in O(q)$, the
orthogonal group of $q$, of dimension $\binom n2$. The deformation space
\[
\Twin(q)=O(q)\backslash\GL(n,\RR)
\]
then has dimension $n^2-\binom n2=\binom{n+1}2$, the dimension of the space $\mathrm{Sym}^2(\RR^{n})^{*}$ of
symmetric bilinear forms; its points are exactly the pulled-back forms $q(g\,\cdot\,,g\,\cdot\,)$ in the
$\GL_n$-orbit of $q$. For $n=4$ this is $10$-dimensional: the flat (pseudo-)metrics. Enlarging $G$ from
$\GL(4,\RR)$ to $\Diff(\RR^{1,3})$ adjoins the translations to the kernel, producing the full Poincar\'e group
of Theorem~\ref{thm:poincare-kernel} and the curved metrics of general relativity in the deformation space.
This is the prototype for the slogan \emph{deformations $=$ metrics $=$ gravitational fields} of Part~II.
\end{example}
...
Assume $\Ker(\alpha) \trianglelefteq G$, so that $\Twin(\alpha) = \Ker(\alpha)\backslash G = G/\Ker(\alpha)$ is a group. There is a bijection between:
...
Two twin operations $\odot_{g_1}$ and $\odot_{g_2}$ are \emph{equivalent under the symmetry group} $\Ker(\alpha)$ --- i.e.\ related by conjugation by an automorphism of $(E,\alpha)$ --- iff $g_1$ and $g_2$ lie in the same double coset $\Ker(\alpha) \backslash G / \Ker(\alpha)$. The number of such equivalence classes is the number of double cosets. (Note that \emph{every} $\odot_g$ is abstractly isomorphic to $\alpha$ by Theorem~\ref{thm:inheritance}; this $\Ker(\alpha)$-equivalence is therefore strictly finer than mere algebraic isomorphism.)
\end{theorem}
...
\end{equation}
This is Einstein velocity addition. The kernel of $\oplus_\phi$ is the identity (trivial for the abelian 1D case). The deformation space $\Twin(\eta_c) = \Poinc(c)\backslash\Diff(\RR^{1,3})$ (right cosets, as in Definition~\ref{def:deformation}) parameterizes all possible metrics (= gravitational fields) on $\RR^{1,3}$.
\end{theorem}
...
\begin{remark}[Branch cuts make $\odot_n$ a partial operation for $n\ge1$]\label{rem:branch}
The group statement in Theorem~\ref{thm:complex-props}(a) must be read locally. For $n\ge1$ the principal
$\log^{(n)}$ is defined only off a union of branch cuts, and the sum $\log^{(n)}(z_1)+\log^{(n)}(z_2)$ need not
lie in the image of $\log^{(n)}$ on $\mathcal D_n$. Hence $\odot_n$ is in general only a \emph{partial} (local)
operation: closure, associativity and inverses hold on the open locus where every iterated principal logarithm
involved stays within its principal domain. Globally one obtains a multivalued structure---naturally a
groupoid rather than a group---while a consistent choice of branches on a simply connected domain recovers an
honest commutative group, which is the sense intended in Theorem~\ref{thm:complex-props}(a). The branch cuts
discussed in Section~\ref{sec:gauge-gravity} are precisely the boundaries of these domains.
\end{remark}
...
\begin{equation}
\mathcal{T}_\CC := \langle T_n : n \in \ZZ \rangle, \qquad T_n: (\CC^{1,3}, \odot_n) \xrightarrow{\sim} (\CC^{1,3}, \odot_{n+1})
\end{equation}
where $T_n(Z) = (\exp(z^0), \exp(z^1), \exp(z^2), \exp(z^3))$ is the componentwise complex exponential. $\mathcal{T}_\CC \cong (\ZZ, +)$ (since $T_{n+1} \circ T_n^{-1} = T_1$ for all $n$).
\end{definition}
